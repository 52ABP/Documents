...
在一个应用程序中，类之间相互依赖。假设我们有一个应用程序服务，使用[仓储](3.3ABP领域层-仓储.md)（repository）类插入[实体](3.1ABP领域层-实体.md)到数据库。在这种情况下，[应用程序服务](4.1ABP应用层-应用服务.md)类依赖于仓储（repository）类。看下例子：
...
Assembly.GetExecutingAssembly()得到一个对包括此代码的程序集的引用。你可以通过RegisterAssemblyByConvention方法注册其他程序集。这同在你的模块初始化（AbpModule.Initialize()）时完成。请查看ABP的[模块系统](1.3ABP总体介绍-模块系统.md)获得更多信息。
